...
- **The view is addressable.** The twelve tabs live in one `<main id="views">` as
  `<section class="view" role="tabpanel">`, and `#tabs` is a `role="tablist"` with roving
  tabindex and Left/Right/Home/End navigation. `show(view)` writes `location.hash`
  (`pushState` for a chosen tab, `replaceState` for the restore) and mirrors it to
  `localStorage.lastView`; a `hashchange` listener follows browser back/forward. On load the
  view resolves **hash → `lastView` → `start`**, so a reload no longer always lands on
  onboarding and a tab can be linked or bookmarked.
- The **"Start Here" onboarding tab** (`data-view="start"`) remains the fallback default. Its
  `makeStart()` controller fetches `GET /api/setup` once on load (and on tab re-open) to render
  live setup-status tiles — CDP up/down plus whether the optional NSE/US data tokens are
  configured. It reads the response's `nse`/`us` keys.
