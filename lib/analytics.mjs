...
// NIFTY daily closes from the NSE data provider (needs the token; degrades honestly).
...
export async function niftyDailyReturns(days = 365) {
  ...
  // NIFTY daily comes from the unauthenticated historical-candle endpoint; no token needed.
  const to = new Date(), from = new Date(Date.now() - days * 864e5);
  const d = x => x.toISOString().slice(0, 10);
  const ik = encodeURIComponent('NSE_INDEX|Nifty 50');
  try {
    const res = await limitedFetch(`https://api.upstox.com/v2/historical-candle/${ik}/day/${d(to)}/${d(from)}`, {
  ...
  if (nd.ok) {
    ...
    if (hmm.ok) {
      ...
      regime = {
        ...
        source: 'NIFTY daily log-returns (NSE data provider), ~1y',
