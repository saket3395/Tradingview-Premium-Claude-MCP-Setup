...
const TF_WEIGHT = { '1M': 0.35, '1W': 0.30, '1D': 0.25, '4H': 0.10, '30m': 0.06, '15m': 0.04 };
const TF_LABEL = { '1M': 'Monthly', '1W': 'Weekly', '1D': 'Daily', '4H': '4-Hour', '30m': '30-Min', '15m': '15-Min' };
...
function keyLevels(tfs, price, dayATR) {
  ...
  for (const a of Object.values(tfs)) {
    if (!a.ok) continue;
    const tw = { '1M': 4, '1W': 3, '1D': 2, '4H': 1, '30m': 0.5, '15m': 0.35 }[a.tf] || 1;
...
export function buildReport(series) {
  const order = ['1M', '1W', '1D', '4H', '30m', '15m'];   // top-down, deliberately
  const tfs = {};
  ...
  const htf = { '1M': 0, '1W': tfs['1M'].ok ? tfs['1M'].dir : 0,
    ...
    '30m': (tfs['4H'].ok ? tfs['4H'].dir : 0), '15m': (tfs['30m'].ok ? tfs['30m'].dir : 0) };
  const all = [];
  for (const tf of order) {
