...
// circuit. The Confirm step swaps in the stock's TRUE circuit from the NSE data provider (lib/upstox).
//
...
import { recordPlan, observePlan } from './journal.mjs';
...
export async function post(region, body, cookie) {
  const headers = {
...
// ---- setup archetype: WHERE in the auction is this symbol? -----------------
// The point is to catch structure BEFORE the range extension prints, not after.
//   OPEN-DRIVE   — early one-sided auction from the open, gap-aligned (highest-
//                  confidence open type; precedes trend days).
//   IB-COIL      — range still compressed (< ~0.9× ATR) but price holding the
//                  directional third with real volume: the pre-breakout state.
//   VALUE-EDGE   — opened beyond the prior close and the pullback is being
//                  accepted (80%-rule style continuation).
//   EXPANSION    — range already >1.3× ATR with price at the extreme: the move
//                  has largely happened. Kept, labeled, and score-penalized.
function classifySetup({ long, O, H, L, C, range, rangeATR, pos, gap, rVol, pdc, minsSinceOpen }) {
  const openPos = range > 0 ? (O - L) / range : 0.5;           // where the open sits in today's range
  const gapAligned = long ? (gap ?? 0) > 0 : (gap ?? 0) < 0;
  const early = minsSinceOpen != null && minsSinceOpen <= 90;
  const oneSidedOpen = long ? openPos <= 0.2 : openPos >= 0.8; // open at the extreme = no trade back through it
  if (early && gapAligned && oneSidedOpen && (rVol ?? 0) >= 1.5) return 'OPEN-DRIVE';
  if (rangeATR < 0.9 && (long ? pos >= 0.7 : pos <= 0.3)) return 'IB-COIL';
  if (pdc != null && gapAligned && (long ? L > pdc : H < pdc)) return 'VALUE-EDGE';
  return 'EXPANSION';
}

// ---- score + build a raw plan for one symbol ------------------------------
// Returns a raw (unfrozen) opportunity or null if not a high-quality setup.
function evaluate(row, idxChg, cfg, m, marketKey, sess) {
  ...
  // ---- pre-expansion logic: archetype + entry quality ------------------------
  const minsSinceOpen = sess && !sess.weekend && sess.curMin >= m.open ? Math.min(sess.curMin, m.close) - m.open : null;
  const setup = classifySetup({ long, O, H, L, C, range, rangeATR, pos, gap, rVol, pdc, minsSinceOpen });

  // entryQuality (0–100): how CHASEABLE is this, right now?
  //  - chase penalty: day range already stretched vs ATR = the extension printed without you
  //  - time decay: same structure at 10:00 is worth more than at 14:30 (fades after the IB)
  //  - volume: participation must be real for a pre-breakout read
  const chasePenalty = clamp((rangeATR - 1.1) / 1.2);
  const timeScore = minsSinceOpen == null ? 0.5 : clamp(1 - Math.max(0, minsSinceOpen - 90) / 240);
  const entryQuality = Math.round(100 * clamp(0.55 * (1 - chasePenalty) + 0.3 * timeScore + 0.15 * vol));

  // Conviction score: direction evidence (loc/acc/vol/idx), R:R geometry and entry
  // quality. Extension (`ext`) no longer earns entry points — an extended day tells
  // you the direction was right, not that the entry is still good.
  const W = { loc: 18, acc: 18, vol: 14, idx: 10, rr: 15, eq: 25 };
  const setupBonus = { 'OPEN-DRIVE': 8, 'IB-COIL': 6, 'VALUE-EDGE': 5, 'EXPANSION': -8 }[setup];
  const score = Math.max(0, Math.min(100, Math.round(
    loc * W.loc + acc * W.acc + vol * W.vol + idx * W.idx + rrScore * W.rr + (entryQuality / 100) * W.eq + setupBonus)));
  if (score < cfg.minScore || rr < cfg.minRR || (rVol ?? 0) < cfg.minRVol) return null;
  if (cfg.minEntryQuality && entryQuality < cfg.minEntryQuality) return null;
  ...
  const bits = [];
  bits.push(`${setup} setup`);
  bits.push(`${long ? 'up' : 'down'} from open (gap ${gap >= 0 ? '+' : ''}${r2(gap)}%)`);
...
export async function scanTPO(cfg = {}, marketKey = 'india') {
  ...
  for (const row of rows) {
    const raw = evaluate(row, idxChg, conf, m, marketKey, sess);
    if (!raw) continue;
    ...
    out.push({
      ...
      setup: plan.setup, entryQuality: plan.entryQuality,
