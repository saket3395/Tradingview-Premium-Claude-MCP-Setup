...
// ---- request throttling ----------------------------------------------------
// One symbol needs five Upstox calls (30m history + today's intraday + day + week +
// month). Fired all at once, and with three analysis tabs each auto-running, that is a
// burst big enough to trip Upstox's rate limiter — which is what produced HTTP 429 on
// every timeframe at once. Requests now queue: a small concurrency cap plus a minimum
// gap between calls, per provider.
...
// Paced for Cloudflare, not for Upstox's documented quota — the observed block is an
// IP-level one that trips far earlier than the API limit suggests.
const upstoxLimit = makeLimiter({ concurrency: 1, minGapMs: 350 });
const alpacaLimit = makeLimiter({ concurrency: 3, minGapMs: 80 });

// ---- circuit breaker -------------------------------------------------------
// api.upstox.com sits behind Cloudflare, and the 429 seen in practice is Cloudflare's
// `error code: 1015` — an IP-level block lasting minutes, not a per-endpoint quota.
// Retrying into it is actively harmful: each retry re-arms the ban. So a 429 trips a
// breaker instead. While open, calls fail instantly with the remaining wait and NO
// network traffic at all, which is what lets the block expire.
const _breaker = {};       // provider -> { until, strikes }
function breakerState(provider) {
  const b = _breaker[provider];
  if (!b || Date.now() >= b.until) return null;
  return Math.ceil((b.until - Date.now()) / 1000);
}
function tripBreaker(provider) {
  const b = _breaker[provider] || { strikes: 0 };
...
class RateLimited extends Error {
  constructor(provider, secs) {
    super(`${provider} is rate limiting this IP (Cloudflare 1015). Pausing all ${provider} requests for ${secs}s — retrying sooner only extends the block.`);
    this.rateLimited = true;
  }
}

// Fetch through the provider's queue. 5xx gets one short retry; 429 trips the breaker
// immediately and is never retried in place.
async function fetchRetry(url, opts, limit, provider) {
  const open = breakerState(provider);
  if (open) throw new RateLimited(provider, open);
  for (let attempt = 1; attempt <= 2; attempt++) {
    const res = await limit(() => fetch(url, opts));
    if (res.status === 429) { tripBreaker(provider); throw new RateLimited(provider, breakerState(provider)); }
...
function flushDisk() {
  ...
  _timer = setTimeout(() => {
    ...
    try {
      ...
      const entries = {};
      for (const [k, v] of _bars) if (v.ttl >= 60 * 60 * 1000 && Date.now() - v.ts < v.ttl) entries[k] = v;
...
// Completed daily/weekly/monthly history does not change during a session; intraday does.
const TTL = { day: 60 * 60 * 1000, week: 60 * 60 * 1000, month: 60 * 60 * 1000,
  '30minute': 10 * 60 * 1000, intraday: 2 * 60 * 1000, alpaca: 30 * 60 * 1000 };
...
async function upstoxCandles(ik, interval, from, to, token) {
  ...
  const bars = (j?.data?.candles || [])
    ...
    .sort((a, b) => a.t - b.t);
  return barsStore(key, bars, TTL[interval] ?? 15 * 60 * 1000);
}
...
async function upstoxIntraday(ik, interval, token) {
  ...
  const res = await fetchRetry(`https://api.upstox.com/v2/historical-candle/intraday/${encodeURIComponent(ik)}/${interval}/`, {
    headers: { authorization: `Bearer ${token}`, accept: 'application/json', 'user-agent': UA },
