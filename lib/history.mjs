...
function ttlFor(interval) {
  const open = nseOpen();
  switch (interval) {
    case 'intraday': return 2 * 60 * 1000;
    case '30minute': return open ? 10 * 60 * 1000 : 12 * HOUR;
    case 'day': return open ? 4 * HOUR : 12 * HOUR;      // Upstox's own max-age is 4h
    case 'week': return open ? 6 * HOUR : 24 * HOUR;
    case 'month': return open ? 12 * HOUR : 24 * HOUR;
    default: return HOUR;
  }
}
