...
//   India (NSE:…)  → NSE data provider historical-candle (same token the TPO Confirm step uses).
...
//   USA (default)  → US market-data provider v2 (4Hour / 1Day / 1Week / 1Month native).
//                    Free IEX feed by default; set US_DATA_FEED=sip if entitled.
//
...
function barsStore(key, bars, ttl, meta = {}) {
  _bars.set(key, { ts: Date.now(), ttl, bars, meta });
...
// ---- India: NSE data provider ----------------------------------------------
function upstoxToken() {
  // New generic name, falling back to the legacy UPSTOX_TOKEN_FILE for existing setups.
...
// The historical-candle endpoints ignore Authorization entirely (verified: identical
// payloads unauthenticated and with a junk bearer). Send it when we have one — a token may
// still buy headroom — but never require it. market-quote/quotes genuinely 401s and keeps
// its own hard check in lib/upstox.mjs.
...
async function upstoxCandles(ik, interval, from, to, token, meta = {}) {
  // NOTE: the key deliberately excludes `from`/`to`. They contain today's date, so
  // including them changed the key at every day rollover and made yesterday's entry
  // unreachable however long its lifetime was. The requested span is fixed per interval,
  // so the window's identity is instrument + interval.
  const key = `u|${ik}|${interval}`;
  ...
  if (!res.ok) throw new Error(j?.errors?.[0]?.message || `NSE data ${interval} HTTP ${res.status}`);
  const bars = (j?.data?.candles || [])
    .map(c => ({ t: Date.parse(c[0]), iso: c[0], o: c[1], h: c[2], l: c[3], c: c[4], v: c[5] }))
    .sort((a, b) => a.t - b.t);
  return barsStore(key, bars, ttlFor(interval), { ...meta, interval, from, to });
}

// Upstox v3 — sub-hour intervals the v2 historical endpoint does not expose. v2 rejects
// 15minute (UDAPI1020: accepts only 1minute/30minute/day/week/month); v3 serves it as
// unit/interval = minutes/15, capped near one month per request. Same candle shape as v2.
async function upstoxCandlesV3(ik, unit, interval, from, to, token, meta = {}) {
  const key = `u3|${ik}|${unit}${interval}`;
  const hit = barsCached(key);
  if (hit) return hit;
  const url = `https://api.upstox.com/v3/historical-candle/${encodeURIComponent(ik)}/${unit}/${interval}/${to}/${from}`;
  const res = await limitedFetch(url, {
    headers: authHeaders(token),
    signal: AbortSignal.timeout(15000),
  }, 'Upstox');
  const j = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(j?.errors?.[0]?.message || `NSE data ${unit}/${interval} HTTP ${res.status}`);
  const bars = (j?.data?.candles || [])
    .map(c => ({ t: Date.parse(c[0]), iso: c[0], o: c[1], h: c[2], l: c[3], c: c[4], v: c[5] }))
    .sort((a, b) => a.t - b.t);
  return barsStore(key, bars, ttlFor('30minute'), { ...meta, interval: `${interval}minute`, from, to });
}

// v3 intraday (today's still-running session) for the sub-hour intervals above.
async function upstoxIntradayV3(ik, unit, interval, token, meta = {}) {
  const key = `ui3|${ik}|${unit}${interval}`;
  const hit = barsCached(key);
  if (hit) return hit;
  if (breakerState('Upstox')) return [];
  const res = await limitedFetch(`https://api.upstox.com/v3/historical-candle/intraday/${encodeURIComponent(ik)}/${unit}/${interval}`, {
    headers: authHeaders(token),
  ...
  return barsStore(key, (j?.data?.candles || [])
    .map(c => ({ t: Date.parse(c[0]), iso: c[0], o: c[1], h: c[2], l: c[3], c: c[4], v: c[5] }))
    .sort((a, b) => a.t - b.t), ttlFor('intraday'), { ...meta, interval: 'intraday' });
}

// Today's (still-running) session — the historical endpoint excludes it.
async function upstoxIntraday(ik, interval, token, meta = {}) {
...
async function indiaSeries(symbol) {
  ...
  if (open) return { ok: false, rateLimited: true, retryInSec: open,
    error: `The NSE data provider is rate limiting this IP (Cloudflare 1015). Analysis paused for ${open}s — data already cached still works.` };
  // Optional, not required: the historical-candle endpoints serve this data
  // unauthenticated. A token is still passed through when configured.
  ...
  if (!ik) return { ok: false, error: `No NSE_EQ instrument found for ${name}.` };
  _ikCache.set(name, ik);
  const meta = { symbol: `NSE:${name}`, market: 'india', ik };
  ...
  const m30 = (async () => {
    const done = await upstoxCandles(ik, '30minute', daysAgo(88), daysAgo(0), token, meta);
    let live = [];
  ...
  // 15-minute via the v3 endpoint (v2 has no 15minute interval), same completed-then-intraday
  // merge as 30m. Window kept at 25 days — safely under v3's ~1-month cap for 15-minute, and
  // ~25 bars/session leaves it far past the 35-bar analysis floor.
  const m15 = (async () => {
    const done = await upstoxCandlesV3(ik, 'minutes', '15', daysAgo(25), daysAgo(0), token, meta);
    let live = [];
    if (nseOpen()) { try { live = await upstoxIntradayV3(ik, 'minutes', '15', token); } catch {} }
    const seen = new Set(done.map(b => b.iso));
    return [...done, ...live.filter(b => !seen.has(b.iso))].sort((a, b) => a.t - b.t);
  })();
  ...
  const jobs = {
    ...
    '30m': m30,
    '4H': m30.then(to4H),
    '1D': upstoxCandles(ik, 'day', daysAgo(450), daysAgo(0), token, meta),   // >252 bars: true 52-week high/low
    '1W': upstoxCandles(ik, 'week', daysAgo(1800), daysAgo(0), token, meta),
    '1M': upstoxCandles(ik, 'month', daysAgo(2500), daysAgo(0), token, meta),
  };
  const tfs = {};
  ...
  return { ok: true, market: 'india',
    source: 'NSE data provider historical-candle (4H aggregated from 30m, NSE session-anchored; weekly/monthly kept current from daily)', tfs };
}

// ---- USA: Alpaca -----------------------------------------------------------
async function alpacaBars(sym, timeframe, start, key, secret, feed) {
  const ck = `a|${sym}|${timeframe}|${feed}`;      // `start` is date-derived — see upstoxCandles
  ...
  const j = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(j?.message || `US data ${timeframe} HTTP ${res.status}`);
  const rows = j?.bars?.[sym] || [];
  if (!rows.length) throw new Error(`US data provider returned no ${timeframe} bars for ${sym}.`);
  return barsStore(ck, rows.map(b => ({ t: Date.parse(b.t), iso: b.t, o: b.o, h: b.h, l: b.l, c: b.c, v: b.v })), TTL.alpaca, { symbol: sym, market: 'usa', interval: timeframe });
}

async function usaSeries(symbol) {
  const open = breakerState('Alpaca');
  if (open) return { ok: false, rateLimited: true, retryInSec: open,
    error: `The US data provider is rate limiting this IP. Analysis paused for ${open}s — data already cached still works.` };
  const key = process.env.US_DATA_KEY_ID || process.env.ALPACA_KEY_ID;
  const secret = process.env.US_DATA_SECRET_KEY || process.env.ALPACA_SECRET_KEY;
  if (!key || !secret) return { ok: false, error: 'No US data credentials — set US_DATA_KEY_ID / US_DATA_SECRET_KEY in .env.' };
  const feed = process.env.US_DATA_FEED || process.env.ALPACA_FEED || 'iex';
  const sym = bareSymbol(symbol);
  const jobs = {
    '15m': alpacaBars(sym, '15Min', daysAgo(30), key, secret, feed),
    '30m': alpacaBars(sym, '30Min', daysAgo(50), key, secret, feed),
  ...
  return { ok: true, market: 'usa', source: `US market-data provider v2 (feed=${feed}; 4H bars are UTC-anchored and include extended hours)`, tfs };
}
...
// ---- cache introspection ---------------------------------------------------
// Powers the Cached Data tab: what is held, how long it stays valid, and — the question
// that actually matters under a rate limit — whether analysing a symbol costs requests.
const INTERVALS_INDIA = ['15minute', '30minute', 'day', 'week', 'month'];

export function cacheStatus() {
  const now = Date.now();
  const bySymbol = new Map();
  let windows = 0;
  for (const [key, v] of _bars) {
    if (now - v.ts >= v.ttl) continue;
    windows++;
    const sym = v.meta?.symbol || key.split('|')[1] || 'unknown';
    if (!bySymbol.has(sym)) bySymbol.set(sym, { symbol: sym, market: v.meta?.market || 'usa', windows: [] });
    bySymbol.get(sym).windows.push({
      interval: v.meta?.interval || key.split('|').pop(),
      bars: v.bars.length,
      ageMin: Math.round((now - v.ts) / 60000),
      expiresInMin: Math.round((v.ts + v.ttl - now) / 60000),
      covers: v.meta?.from ? `${v.meta.from} → ${v.meta.to}` : null,
    });
  }
  const rows = [...bySymbol.values()].map(r => {
    r.windows.sort((a, b) => a.interval.localeCompare(b.interval));
    const c = estimateCostSync(r.symbol, r.market);
    return { ...r, nextAnalysisCost: c.cost, free: c.cost === 0, missing: c.missing };
  }).sort((a, b) => a.symbol.localeCompare(b.symbol));

  return {
    ok: true, ts: now,
    session: { nseOpen: nseOpen(), nextOpenInMin: Math.round(msToNextOpen() / 60000) },
    totals: { symbols: rows.length, windows, freeSymbols: rows.filter(r => r.free).length },
    symbols: rows,
  };
}

// What a fresh analysis of `symbol` would cost in upstream requests, right now.
function estimateCostSync(symbol, market) {
  const missing = [];
  if (market === 'india') {
    const ik = _ikCache.get(bareSymbol(symbol));
    if (!ik) return { cost: null, missing: ['instrument not resolved yet'] };
    for (const iv of INTERVALS_INDIA) if (!barsCached(`u|${ik}|${iv}`)) missing.push(iv);
    if (nseOpen() && !barsCached(`ui|${ik}|30minute`)) missing.push('intraday');
  } else {
    const feed = process.env.US_DATA_FEED || process.env.ALPACA_FEED || 'iex';
    for (const tf of ['4Hour', '1Day', '1Week', '1Month']) {
      if (!barsCached(`a|${bareSymbol(symbol)}|${tf}|${feed}`)) missing.push(tf);
    }
  }
  return { cost: missing.length, missing };
}

// Instrument keys seen so far, so a cost estimate needs no network of its own.
const _ikCache = new Map();

export async function estimateCost(symbol) {
  const india = isIndia(symbol) || (!/:/.test(String(symbol)) && !!(await instrumentKeyFor(bareSymbol(symbol)).catch(() => null)));
  const market = india ? 'india' : 'usa';
  if (india) {
    const ik = await instrumentKeyFor(bareSymbol(symbol)).catch(() => null);
    if (ik) _ikCache.set(bareSymbol(symbol), ik);
  }
  const c = estimateCostSync(india ? `NSE:${bareSymbol(symbol)}` : bareSymbol(symbol), market);
  const blocked = breakerState(india ? 'Upstox' : 'Alpaca');   // internal breaker key
  const providerLabel = india ? 'NSE data' : 'US data';        // user-facing label
  return {
    ok: true, symbol: india ? `NSE:${bareSymbol(symbol)}` : bareSymbol(symbol), market,
    cost: c.cost, free: c.cost === 0, missing: c.missing,
    provider: providerLabel, blockedForSec: blocked,
    verdict: c.cost === 0 ? 'Fully cached — analysing this costs no upstream requests.'
      : blocked ? `Would need ${c.cost} request(s), but ${providerLabel} is rate limiting for another ${blocked}s.`
        : `Would fetch ${c.cost} window(s): ${c.missing.join(', ')}.`,
  };
}
