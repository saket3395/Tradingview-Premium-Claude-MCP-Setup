...
// Resolved LAZILY, on first use: server.mjs loads .env in its module body, which runs
// AFTER every import is evaluated, so reading process.env at module scope silently used
// the defaults and ignored the operator's tuning entirely.
const num = (v, d) => (Number(v) > 0 ? Number(v) : d);
let LIMITS = null;
function limits() {
  return LIMITS ||= {
    Upstox: {
      // New generic names, falling back to the legacy UPSTOX_* names for existing setups.
      concurrency: num(process.env.NSE_CONCURRENCY || process.env.UPSTOX_CONCURRENCY, 1),
      minGapMs: num(process.env.NSE_MIN_GAP_MS || process.env.UPSTOX_MIN_GAP_MS, 600),
      maxPerMin: num(process.env.NSE_MAX_PER_MIN || process.env.UPSTOX_MAX_PER_MIN, 60),
    },
...
//
// Pass `timeoutMs`, never a pre-built `signal: AbortSignal.timeout(...)`. A signal
// created by the caller starts counting the moment it is constructed — which is BEFORE
// the request is queued here. At a 1200ms gap and a 25/min ceiling a request can sit in
// the queue for minutes, so the budget was spent waiting and the fetch was aborted before
// it ever left. That surfaced as "The operation was aborted due to timeout" on every
// timeframe for a handful of symbols per scan, and it got worse the more conservatively
// the limiter was tuned — the safer the pacing, the more names silently dropped.
// Building the signal inside the queued job makes the timeout mean what it says: time
// allowed for the request, not time spent waiting for a turn.
export async function limitedFetch(url, opts = {}, provider = 'Upstox') {
  ...
  const withTimeout = () => {
    ...
    // Honour a caller-supplied signal too (scan cancellation), without letting it start
    // the clock early.
    return { ...init, signal: init.signal ? AbortSignal.any([init.signal, t]) : t };
  };
  const limit = limiterFor(provider);
  for (let attempt = 1; attempt <= 2; attempt++) {
    const res = await limit(() => fetch(url, withTimeout()));
...
// Snapshot for the Cached Data tab: what each provider is allowed and whether it is
// currently blocked.
// Neutral, user-facing labels for the internal provider keys (shown in the Cached Data tab).
// The internal keys stay as-is so the breaker/limiter coupling across callers is untouched.
const PROVIDER_LABEL = { Upstox: 'NSE data', Alpaca: 'US data' };

export function providerStatus() {
  return Object.entries(limits()).map(([name, l]) => ({
    provider: PROVIDER_LABEL[name] || name, blockedForSec: breakerState(name),
    concurrency: l.concurrency, minGapMs: l.minGapMs, maxPerMin: l.maxPerMin,
  }));
}
