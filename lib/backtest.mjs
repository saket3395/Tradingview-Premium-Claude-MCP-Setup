...
export async function runBacktest(gateCfg, limit = 100) {
  // Historical candles need no credential; the token is sent only if one is configured.
  ...
  for (const t of india) {
    ...
    if (!ik) { errors.push(`${t.symbol}: no NSE instrument (run a Confirm once to build the cache)`); continue; }
    try {
  ...
  return {
    ok: true, ts: Date.now(), market: 'india', resolution: '1-minute (NSE data provider historical)',
    ...
    note: stopped
      ...
      : bounded > 0
        ? `Replayed the ${india.length} most recent India plans; ${bounded} older ones were left out (each plan costs one NSE-provider request). Raise testing.backtestLimit in config/markets.json to widen it.`
        : undefined,
