...
export async function getCircuit(symbol) {
  ...
  if (!token) return { ok: false, reason: 'no_token', error: 'No Upstox token — point UPSTOX_TOKEN_FILE at an Analytics token (read-only, ~1yr validity, no daily refresh).' };
  ...
  try {
    ...
    if (res.status === 401 || j?.errors?.some(e => /token/i.test(e.message || ''))) {
      return { ok: false, reason: 'stale_token', error: 'Upstox token invalid/expired — replace it with an Analytics token (~1yr validity); plain OAuth tokens expire daily 03:30 IST.' };
    }
