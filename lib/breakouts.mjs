...
// `onProgress` receives {phase,...} events as the deep pass walks the shortlist, and
// `signal` lets a caller abort mid-scan (an SSE client closing the connection). Both are
// optional: without them this behaves exactly as the plain JSON endpoint always did.
export async function scanBreakouts({ engine = 'patterns', market = 'india', tf = '1D',
  patternType = 'all', cfg = {}, candidates, onProgress, signal } = {}) {
  ...
  // Never throws into the scan: a broken progress sink must not fail the work.
  ...
  emit({ phase: 'shortlist', universe: pre.universe, prefiltered: pre.prefiltered,
         candidates: cands.length, dataStatus: pre.dataStatus, marketLabel: m.label });

  const rows = [], skipped = [];
  let stoppedEarly = null, analysed = 0, processed = 0, aborted = false;

  // One candidate. Returns 'ok' | 'skip' | 'halt' so the caller can report progress on
  // EVERY candidate, including the ones that skip. Emitting only on success would freeze
  // the progress bar for any run where names legitimately drop out (no history, bad data,
  // a missing provider token) — which is most of them.
  const runOne = async (c) => {
    let series;
    try { series = await getSeries(c.ticker); }
    catch (e) { skipped.push(`${c.symbol}: ${e.message}`); return 'skip'; }
    if (!series.ok) {
      skipped.push(`${c.symbol}: ${series.error}`);
      if (series.rateLimited) {
        stoppedEarly = `${m.providerLabel} is rate limiting — stopped after ${analysed} of ${cands.length} candidates.`;
        return 'halt';
      }
      return 'skip';
    }
    ...
    for (const r of out.rows) {
      const row = { ...r, fromHighPct: c.fromHighPct, high52: c.high52, changePct: c.changePct, scanRVol: c.rVol, perf6M: c.perf6M };
      rows.push(row);
      emit({ phase: 'row', row });     // so the table fills as the scan walks
    }
    return 'ok';
  };

  for (const c of cands) {
    // The client went away (tab closed, Cancel pressed). Stop burning provider quota on a
    // result nobody will read — a full pass is minutes of upstream calls.
    ...
    const verdict = await runOne(c);
    processed++;
    emit({ phase: 'symbol', symbol: c.symbol, verdict,
           processed, analysed, total: cands.length, found: rows.length });
    if (verdict === 'halt') break;
  }
  if (aborted) stoppedEarly = `Scan cancelled after ${processed} of ${cands.length} candidates.`;
  ...
  const result = {
    ...
    candidates: cands.length, analysed, processed, count: rows.length, tierCounts,
    // `count` is every qualifier; `rows` may be fewer. The header reported the former
    // and the table rendered the latter with nothing acknowledging the gap — "found 50"
    // above a 25-row table. `returned` and `truncated` make the difference explicit.
    rows: rows.slice(0, conf.top), returned: Math.min(rows.length, conf.top),
    truncated: rows.length > conf.top,
    skipped: skipped.slice(0, 60), stoppedEarly,
    thresholds: {
  ...
  // user hitting Scan again wants.
  //
  // A pass that walked the whole shortlist but analysed NOTHING is not a result either.
  // It happens when every candidate fails before its detector runs — a missing provider
  // credential being the obvious case — and it completes in milliseconds, so `stoppedEarly`
  // is null and it looked "complete" to the cache. The effect was that fixing the
  // underlying cause changed nothing for ten minutes: Scan kept serving the cached zero.
  // An empty answer is only worth caching when it was actually computed.
  const analysedNothing = cands.length > 0 && analysed === 0;
  if (!stoppedEarly && !aborted && !analysedNothing) _cache.set(key, { ts: Date.now(), result });
  return result;
}
