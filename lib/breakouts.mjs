...
//
// WHY THE SHORTLIST IS SMALL. A fresh NSE symbol costs up to five Upstox requests, and
// Upstox's Cloudflare block trips well below the documented quota (see lib/history.mjs).
// `candidates` (config breakouts.candidates, default 10) is therefore a deliberate cap,
// the deep pass runs SEQUENTIALLY through the shared rate limiter, and if the breaker
// opens mid-scan the scan STOPS and says so rather than pretending the result is complete.
//
...
const REGIONS = {
  india: { region: 'india', label: '🇮🇳 NSE', exchanges: ['NSE'], needsAuth: false, provider: 'Upstox' },
  usa: { region: 'america', label: '🇺🇸 US', exchanges: ['NASDAQ', 'NYSE', 'AMEX'], needsAuth: true, provider: 'Alpaca' },
};
...
function fromPatterns(series, symbol, tf, patternType, conf) {
...
async function fromVCP(series, symbol, conf) {
...
function fromElliott(series, symbol, tf, conf) {
...
export async function scanBreakouts({ engine = 'patterns', market = 'india', tf = '1D',
  ...
  const conf = {
    ...
    candidates: Math.max(1, Math.min(25, candidates ?? cfg.candidates ?? 10)),
    top: cfg.top ?? 25,
  };
  // VCP is a daily-base method by construction, so the timeframe filter cannot apply to it.
  const effTf = engine === 'vcp' ? '1D' : tf;

  const key = `${engine}|${market}|${effTf}|${patternType}|${conf.candidates}`;
  ...
  for (const c of cands) {
    ...
    if (open) { stoppedEarly = `${m.provider} is rate limiting — stopped after ${analysed} of ${cands.length} candidates (retry in ${open}s).`; break; }
    let series;
    try { series = await getSeries(c.ticker); } catch (e) { skipped.push(`${c.symbol}: ${e.message}`); continue; }
    if (!series.ok) {
      skipped.push(`${c.symbol}: ${series.error}`);
      if (series.rateLimited) { stoppedEarly = `${m.provider} is rate limiting — stopped after ${analysed} of ${cands.length} candidates.`; break; }
      continue;
    }
    analysed++;
    let out;
    try {
      out = engine === 'vcp' ? await fromVCP(series, c.symbol, conf)
        : engine === 'elliott' ? fromElliott(series, c.symbol, effTf, conf)
          : fromPatterns(series, c.symbol, effTf, patternType, conf);
  ...
  // Closest to breakout first; ties broken by the engine's own quality score.
  rows.sort((a, b) => a.distPct - b.distPct || (b.score ?? 0) - (a.score ?? 0));
  ...
  const result = {
    ...
    candidates: cands.length, analysed, count: rows.length,
    ...
    thresholds: {
      ...
      minConfidence: conf.minConfidence, candidates: conf.candidates,
    ...
    note: [
      ...
      + `Stage 2 ran real ${engine === 'vcp' ? 'Minervini VCP' : engine === 'elliott' ? 'Elliott Wave' : 'pattern'} analysis on the top ${cands.length}.`,
